    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    <x-nav-link :href="route('tickets.index')" :active="request()->routeIs('tickets.index')">
                        {{ auth()->user()->role === 'agent' ? __('Ticket Queue') : __('My Tickets') }}
                    </x-nav-link>
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <span class="me-3 px-2.5 py-0.5 rounded-full text-xs font-bold uppercase tracking-wider {{ auth()->user()->role === 'agent' ? 'bg-indigo-100 text-indigo-700' : 'bg-emerald-100 text-emerald-700' }}">
                    {{ auth()->user()->role }}
                </span>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1 border-t border-gray-100">
            <x-responsive-nav-link :href="route('tickets.index')" :active="request()->routeIs('tickets.index')">
                {{ auth()->user()->role === 'agent' ? __('All Tickets') : __('My Tickets') }}
        ...
        <div class="pt-4 pb-1 border-t border-gray-200 bg-gray-50">
            <div class="px-4 flex items-center justify-between">
                ...
                <span class="px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-widest bg-white border border-gray-200">
                    {{ auth()->user()->role }}
                </span>
            </div>
